...
_playerIcon = "";
_playerID = [_x, false, false] call ace_common_fnc_getName;
...
if (_playermed == 1) then {_playerIcon = _playerIcon + "<font color='#aa2525' img image='\z\ace\addons\zeus\ui\Icon_Module_plt0A_Medic_ca.paa' width='16' height='16'/></font>"};
...
switch (true) do
   {
      case (["Zeus", (_grpname)] call BIS_fnc_inString): {_plt0A pushbackUnique _playername};
      case (["0-A", (_grpname)] call BIS_fnc_inString): {_plt0A pushbackUnique _playername};
...
if (count _plt0A > 0) then {
   _plt0ARoster = "<font color='#bc9ae1'>Zero Alpha</font><br/>"; 
   _plt0AGRP = ""; 
   _plt0AGRP = _plt0AGRP + (_plt0A joinString ",  ");
   _plt0ARoster = _plt0ARoster + _plt0AGRP;
   _plt0ARoster = _plt0ARoster + "<br/><br/>";
   _platrosterfinal = _platrosterfinal + _plt0ARoster;
};
